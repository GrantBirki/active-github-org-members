...
describe GitHub do
  ...
  let(:app_key) { OpenSSL::PKey::RSA.generate(2048).to_pem }
  ...
  describe "#initialize" do
    ...
    it "loads app key from .pem file path" do
      pem_file = Tempfile.new(["app-key", ".pem"])
      pem_file.write(app_key)
      pem_file.close

      begin
        github = GitHub.new(app_id: 999, installation_id: 888, app_key: pem_file.path)
    ...
    context "error handling" do
      ...
      it "raises error when app key file is empty" do
        empty_file = Tempfile.new(["empty-key", ".pem"])
